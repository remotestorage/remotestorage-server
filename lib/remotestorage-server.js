...
exports.createServer = function(specVersion, tokenStore, dataStore) {
  ...
  function writeHead(res, status, origin, etag, contentType, contentLength) {
    var headers = {
  ...
  function writeRaw(res, contentType, content, origin, etag) {
    writeHead(res, 200, origin, etag, contentType, content.length);
    res.write(content);
    res.end();
  }
  ...
  function doGet(req, res, path) {
    if (path.substr(-1) === '/') {
      writeRaw(res, folderContentType, JSON.stringify(mainInstance.getFolderDescription(path, folderFormat)), req.headers.origin, mainInstance.getVersion(path));
    } else {
  ...
  function doPut(req, res, path) {
    ...
    req.on('end', function() {
      ...
      mainInstance.set(path, dataStr, req.headers['content-type'], etag);
      writeRaw(res, undefined, undefined, req.headers.origin, etag);
    });
  }
