...
console.log('condeMet', cond, revision);
    var i, found;
    if (err) {
      cb(err);
      return;
    ...
    if (cond.ifNoneMatch === '*') {//if-none-match is either '*'...
      ...
      for (i=0; i<cond.ifNoneMatch.length; i++) {
        console.log('compare', cond.ifNoneMatch[i].toString('utf-8', revision.toString('utf-8')));
    ...
    if (cond.ifMatch) {
      for (i=0; i<cond.ifMatch.length; i++) {
        console.log('compare', cond.ifMatch[i].toString('utf-8', revision.toString('utf-8')));
...
module.exports.prototype.setParents = function(username, pathParts, fileItself, revision, cb) {
  if (pathParts.length <= 1) {
    ...
    this.dataStore.get(username, 'content:' + pathParts.join('/') + '/', function(err1, obj) {
      if (err1) {
        cb(err1);
        return;
      }
...
module.exports.prototype.set = function(username, path, dataBuf, contentType, revision, cb) {
  this.dataStore.set(username, 'content:' + path, dataBuf, function(err1) {
    ...
    this.dataStore.set(username, 'contentType:' + path, new Buffer(contentType, 'utf-8'), function(err2) {
      if (err2) {
        cb(err2);
        return;
      }
      this.dataStore.set(username, 'revision:' + path, new Buffer(revision, 'utf-8'), function(err3) {
        if (err3) {
          cb(err3);
          return;
        }
