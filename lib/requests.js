...
module.exports.prototype.checkMayRead = function(req, res, username, path, cb) {
  this.scopesInstance.mayRead(username, req.headers.authorization, path, function(err, answer) {
    if (err) {
...
module.exports.prototype.checkMayWrite = function(req, res, username, path, cb) {
  this.scopesInstance.mayWrite(username, req.headers.authorization, path, function(err, answer) {
    if (err) {
      this.error(res, err, req.headers.origin);
    } else if (answer) {
      cb(err, answer);
