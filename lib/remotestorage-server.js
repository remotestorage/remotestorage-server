  ...
  function storage(req, res) {
    ...
    if(req.method=='OPTIONS') {
      ...
      if(!mayRead(req.headers.authorization, path)) {
        ...
        if(dataStore.get('content:'+path)) {
          if(path.substr(-1)=='/') {
            writeRaw(res, undefined, undefined, req.headers.origin, dataStore.get('version:'+path), cond);
          } else {
            writeRaw(res, dataStore.get('contentType:'+path), '', req.headers.origin, dataStore.get('version:'+path), cond);
          }
        } else {
          if(path.substr(-1) == '/') {
            writeRaw(res, undefined, undefined, req.headers.origin, getVersion(path), cond);
          } else {
      ...
      if(!mayRead(req.headers.authorization, path)) {
        ...
        if(dataStore.get('content:'+path)) {
          ...
          if(path.substr(-1) == '/') {//empty dir
            writeJson(res, folderContentType, toFolderDescription({}), req.headers.origin, getVersion(path), cond);
