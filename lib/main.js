...
module.exports.prototype.setParents = function(username, pathParts, fileItself, revision, cb) {
  if (pathParts.length <= 1) {
    ...
    this.dataStore.get(username, 'content:' + pathParts.join('/') + '/', function(err1, obj) {
      ...
      this.dataStore.set(username, 'content:' + pathParts.join('/') + '/', JSON.stringify(obj), function(err2) {
        if (err2) {
          cb(err2);
          return;
        }
        this.dataStore.set(username, 'revision:' + pathParts.join('/') + '/', sha1(JSON.stringify(obj)), function(err3) {
          if (err3) {
            cb(err3);
            return;
          }
