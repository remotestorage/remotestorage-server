 ...
 * the queue
 */
module.exports.prototype._lock = function(res, cb) {
  this._queue.push(function(release) {
    res.end = (function(origEnd) {
      return function(content) {
        origEnd.bind(this)(content);
        release();
      };
    })(res.end);
    cb(res);
...
module.exports.prototype.handleRequest = function(req, res, folderFormat, folderContentType) {
  ...
  this._lock(res, function(_res) {
    switch (req.method) {
    case 'OPTIONS':
      this.writeRaw(_res, undefined, undefined, req.headers.origin);
      break;
    case 'HEAD':
      this.checkMayRead(req, _res, username, path, function() {
        this.checkFound(req, _res, username, path, function() {
          this.checkCondMet(req, _res, username, path, 304, function() {
            this.doHead(req, _res, username, path, folderContentType);
          }.bind(this));
        }.bind(this));
      }.bind(this));
      break;
    case 'GET':
      this.checkMayRead(req, _res, username, path, function() {
        this.checkFound(req, _res, username, path, function() {
          this.checkCondMet(req, _res, username, path, 304, function() {
            this.doGet(req, _res, username, path, folderFormat, folderContentType);
          }.bind(this));
        }.bind(this));
      }.bind(this));
      break;
    case 'PUT':
      this.checkNoFolder(req, _res, username, path, function() {
        this.checkMayWrite(req, _res, username, path, function() {
          this.checkCondMet(req, _res, username, path, 412, function() {
            this.doPut(req, _res, username, path);
          }.bind(this));
        }.bind(this));
      }.bind(this));
      break;
    case 'DELETE':
      this.checkNoFolder(req, _res, username, path, function() {
        this.checkMayWrite(req, _res, username, path, function() {
          this.checkCondMet(req, _res, username, path, 412, function() {
            this.checkFound(req, _res, username, path, function() {
              this.doDelete(req, _res, username, path);
            }.bind(this));
          }.bind(this));
        }.bind(this));
      }.bind(this));
      break;
    default:
      this.respond(_res, req.headers.origin, 405);
    }
  }.bind(this));
};
