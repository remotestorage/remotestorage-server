...
exports.createInstance = function(pathPrefix, scopesInstance, mainInstance) {
  ...
  function writeRaw(res, contentType, content, origin, revision, explicitLength) {
    console.log('writeRaw(res', contentType, content, origin, revision);
    if (Buffer.isBuffer(content)) {
      console.log('got content buffer of length', content.length);
    } else {
      console.log('no content buffer', typeof(content));
    }
    console.log('writeHead(res', 200, origin, revision, contentType, (Buffer.isBuffer(content) ? content.length : explicitLength));
    writeHead(res, 200, origin, revision, contentType, (Buffer.isBuffer(content) ? content.length : explicitLength));
    if (Buffer.isBuffer(content)) {
      console.log('writing buffer', content.toString());
      res.end(content);
    } else {
      console.log('no buffer', typeof(content));
      res.end();
    }
  }
